...
const AdminSidebar = ({ active, setActive, isCollapsed, setIsCollapsed }) => {
  const { setUser, setIsLogin } = useAuth();
  const navigate = useNavigate();

  const menuItems = [
    { key: "overview", title: "Overview", icon: <PiSquaresFourBold /> },
    { key: "people", title: "Users & Partners", icon: <HiOutlineUsers /> },
    { key: "orders", title: "Orders", icon: <GiShoppingCart /> },
  ];
          ...
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {!isCollapsed && (
              <div className="min-w-0">
                ...
                <p className="truncate text-sm font-black text-(--color-text)">Dashboard</p>
              </div>
            )}
          </div>
        </div>
